...
def to_filehandle(fname, flag='r', return_opened=False):
  ...
  *fname* can be a filename or a file handle.  Support for gzipped
  ...
  if is_string_like(fname):
    kwargs_enc={}
    if 'b' not in flag: kwargs_enc['encoding']='utf-8'
    if fname.endswith('.gz'):
      import gzip
      fh = gzip.open(fname, flag, **kwargs_enc)
    elif fname.endswith('.bz2'):
      import bz2
      fh = bz2.BZ2File(fname,flag, **kwargs_enc)
    else:
      fh = open(fname, flag, **kwargs_enc)
    opened = True
